    ...
    def connecter_compte(self, username, password):
        ...
        if result:
            ...
            if bcrypt.checkpw(password.encode('utf-8'), stored_hashed_password):
                self.open_main_app(password, result[0])  # Connexion réussie
            else:
                messagebox.showerror("Erreur", "Nom d'utilisateur ou mot de passe incorrect.")
        else:
            messagebox.showerror("Erreur", "Nom d'utilisateur ou mot de passe incorrect.")

    def open_main_app(self, password, user_id=None):
        # Assure la fermeture propre de la fenêtre principale
        self.destroy()
        app = MainApp(user_id, password)
        app.mainloop()
...
class MainApp(ctk.CTk):
    def __init__(self, user_id, user_password):
        ...
        self.update = False
        print(self.update, self.user_id)
    ...
    # Générer une clé de chiffrement (à ne faire qu'une seule fois et à stocker en lieu sûr)
    ...
    def encrypt_data(self, data):
        key = self.load_key()  # Charger la clé
        fernet = Fernet(key)
        encrypted_data = fernet.encrypt(data.encode())  # Chiffrer les données (en UTF-8)
    ...
    def decrypt_data(self, encrypted_data):
        key = self.load_key()  # Charger la clé
        fernet = Fernet(key)
        decrypted_data = fernet.decrypt(encrypted_data).decode()  # Déchiffrer les données et les décoder en UTF-8
    ...
    def search(self, event):
        ...
        if not search_value:
            if not self.update:
                print(f"self.update = {self.update}")
                results = self.get_all_passwords()
                # self.update = True
                print("no search : self.update = True")
            else:
                return
        else:
            ...
            cursor.execute('''
                ...
                WHERE LOWER(service_name) LIKE LOWER(?) OR LOWER(password) LIKE LOWER(?) AND user_id = ?
            ''', ('%' + search_value + '%', '%' + search_value + '%', self.user_id))

            # Récupérer les résultats
            results = cursor.fetchall()
            print(f"self.update = {self.update}")
            self.update = False
            print("serche : self.update = False")
    ...
    def add_password(self, service_name, password):
        cursor.execute("INSERT INTO passwords (user_id, service_name, password) VALUES (?, ?, ?)",
                       (self.user_id, service_name, password))
        conn.commit()
    ...
    def show_passwords(self, passwords):
        print(f"show password, update : {self.update}")
        print(passwords)
